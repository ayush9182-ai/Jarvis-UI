...
app.post("/v1/assistant", async (req, res) => {
  try {
    ...
    const upstream = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${process.env.GEMINI_API_KEY}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ contents: [{ parts: [{ text: `You are Jarvis. Address the user as Boss. Handle conversation, reasoning and code. Reply in natural Hinglish unless requested otherwise. Memory:\n${memory}\nUser:\n${message}` }] }] }) });
  ...
  } catch (error) { console.error(error); res.status(500).json({ error: "Assistant unavailable" }); }
});
