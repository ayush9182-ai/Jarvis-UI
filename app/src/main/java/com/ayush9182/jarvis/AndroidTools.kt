...
class AndroidTools(private val context: Context, private val confirmation: ConfirmationPolicy) : JarvisTool {
    ...
    private val packages = mapOf(
        ...
        "chrome" to "com.android.chrome", "maps" to "com.google.android.apps.maps",
        "camera" to "com.android.camera2", "telegram" to "org.telegram.messenger",
        "gmail" to "com.google.android.gm", "settings" to "com.android.settings"
    )

    override fun canHandle(route: IntentRouter.Route) = route.type != IntentRouter.IntentType.CHAT && route.type != IntentRouter.IntentType.GREETING
    ...
    override fun run(route: IntentRouter.Route, callback: (String) -> Unit) {
        when (route.type) {
            ...
            IntentRouter.IntentType.SEARCH -> openUrl("https://www.google.com/search?q=${URLEncoder.encode(route.value, "UTF-8")}", "Web search khol diya, Boss.", callback)
            IntentRouter.IntentType.SETTINGS -> { context.startActivity(Intent(Settings.ACTION_SETTINGS)); callback("Settings open kar diya, Boss.") }
    ...
    private fun openApp(name: String, callback: (String) -> Unit) {
        ...
        if (intent != null) { context.startActivity(intent); callback("${match.key} open kar diya, Boss.") }
        else openUrl("https://www.google.com/search?q=${URLEncoder.encode(name, "UTF-8")}", "App nahi mila, web search khol diya, Boss.", callback)
    }

    private fun call(target: String, callback: (String) -> Unit) {
        val number = target.filter { it.isDigit() || it == '+' }
        if (number.length < 7) { callback("Boss, number format use kijiye: call 9876543210"); return }
    ...
    private fun message(target: String, callback: (String) -> Unit) {
        ...
        if (number.length < 7) { callback("Format: message 9876543210 hello Boss"); return }
        ...
        confirmation.confirm("Message $number?", "SMS composer khulega. Send karne se pehle aap confirm karoge.") {
            context.startActivity(Intent(Intent.ACTION_SENDTO, Uri.parse("smsto:$number")).apply { putExtra("sms_body", body) })
            callback("SMS draft ready hai, Boss.")
        }
    }

    private fun openUrl(url: String, result: String, callback: (String) -> Unit) { context.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(url))); callback(result) }
}
